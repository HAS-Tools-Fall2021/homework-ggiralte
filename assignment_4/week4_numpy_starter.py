...
flow_test = flow_data[:, 3] < 85  # Note that this returns a 1-d array that has an entry for every day of the timeseies (i.e. row) with either a true or a fals
month_test = flow_data[:, 1] == 9   # doing the same thing but testing if month =7 
year_test = flow_data[:, 0] == 2012
combined_test = flow_test & month_test & year_test # now looking at our last two tests and finding when they are both true
flow_count = np.sum(combined_test) # add up all the array (note Trues = 1 and False =0) so by default this counts all the times our criteria are true
print(flow_count)
